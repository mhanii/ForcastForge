...
class WeatherView(View):
    ...
    def get_openweather(self, ip, city):
        response = requests.post("http://ip-api.com/batch", json=[{"query": ip},]).json()

        lat = response[0]['lat']
        lon = response[0]['lon']
        # #print(lat, lon)
        # Replace with your OpenWeatherMap API key
        api_key = '74b5e0472627fce02c30bb42e08668f6'
    ...
    def get_scraper_weather(self, ip, city):
        ...
        temp_response = requests.post("http://ip-api.com/batch", json=[{"query": ip}]).json()
        lat = temp_response[0]['lat']
        lon = temp_response[0]['lon']
        timezone = temp_response[0]['timezone']
